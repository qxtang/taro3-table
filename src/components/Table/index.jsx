...
const Table = (props) => {
    ...
    useEffect(() => {
        // 查找需要排序的列
        const sortColumns = columns.filter(item => item.sortOrder) || [];
     ...
     * @param index
     */
    const handleClickTitle = useCallback((item, index) => {
        ...
        // 连续点击循环设置排序方式
        const array = ['ascend', 'descend', undefined];
        const curr = array.indexOf(temp[index].sortOrder);
        const next = temp[index].sortOrder = array[(curr + 1) % array.length];
     ...
     * @param sortOrder
     */
    const compare = (a, b, sortOrder = 'ascend') => {
    ...
    const getSize = (size) => {
        if (typeof size === 'number') {
            return Taro.pxTransform((size) * 2);
        } else {
            return String(size);
        }
    };
     ...
     * @param index
     */
    const getFixedDistance = useCallback((fixedType, index) => {
        let result;
        if (fixedType === 'left') {
    ...
    const Title = (props) => {
        ...
        return (
            <View
                ...
                style={{
                    [column.fixed]: column.fixed && getFixedDistance(column.fixed, index),
                        ...
                        return (
                            <View
                                ...
                                className={classnames({
                                    ...
                                    'expansion': expansion,
                                    [columnItem.className]: true
                                })}
                                style={{
                                    textAlign: columnItem.align || 'center',
                                    width: getSize(columnItem.width || DEFAULT_COL_WIDTH),
                                    [columnItem.fixed]: columnItem.fixed && getFixedDistance(columnItem.fixed, colIndex),
                                    ...columnItem.style,
                                    ...colStyle
                                }}
                            >{result}</View>
